        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-2xl p-6 shadow-lg">
                <div class="flex items-center gap-3 mb-4">
                    <div class="h-10 w-10 rounded-xl flex items-center justify-center" style="background: linear-gradient(135deg, #3b82f6, #6366f1);">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    </div>
                    <div>
                        <h3 class="text-lg font-bold text-blue-900">Upcoming Events</h3>
                        <p class="text-sm text-blue-700">Meetings, elections & announcements</p>
                    </div>
                </div>
                <div class="space-y-3">
                    @foreach($upcomingEvents as $event)
                        @php
                            $eventIcon = match($event->type) {
                                ...
                                'election' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>',
                                'offering' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>',
                                default => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>'
                            };
                            $priorityBadge = match($event->priority ?? 'normal') {
                        ...
                        @endphp
                        <div class="bg-white rounded-xl p-4 border border-blue-100 hover:shadow-md transition">
                            <div class="flex items-start gap-3">
                                <div class="h-9 w-9 rounded-lg flex items-center justify-center flex-shrink-0" style="background: linear-gradient(135deg, #dbeafe, #e0e7ff);">
                                    <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $eventIcon !!}</svg>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-start justify-between gap-2 mb-1">
                                        <h4 class="text-sm font-bold text-slate-800">{{ $event->title }}</h4>
                                        @if($event->priority && $event->priority !== 'normal')
                                            <span class="text-xs font-semibold px-2 py-0.5 rounded-full border {{ $priorityBadge }}">{{ ucfirst($event->priority) }}</span>
                                        @endif
                                    </div>
                                    @if($event->description)
                                        <p class="text-xs text-slate-600 mb-2">{{ Str::limit($event->description, 80) }}</p>
                                    @endif
                        ...
                        <div class="bg-white rounded-xl p-4 border border-orange-100 hover:shadow-md transition">
                            <div class="flex items-start justify-between gap-3 mb-2">
                                <div class="flex items-center gap-3">
                                    <div class="h-9 w-9 rounded-lg flex items-center justify-center flex-shrink-0 {{ $urgencyClass }}">
                                        <svg class="w-5 h-5 {{ $urgencyText }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                    </div>
                                    <div>
                                        <h4 class="text-sm font-bold text-slate-800">Loan #{{ $loan->id }}</h4>
                                        <p class="text-xs text-slate-500">{{ $loan->loan_type ?? 'Personal Loan' }}</p>
                                    </div>
                                </div>
