...
<div class="space-y-6">
    <div class="relative overflow-hidden rounded-lg p-6 lg:p-8" style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #334155 100%);">
        <div class="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('admin.reports.schedule.contributions.export', ['year' => $year]) }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-white/15 hover:bg-white/25 border border-white/20 text-white text-sm font-medium rounded-xl">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden print:shadow-none">
        <div class="p-6 print:p-4 overflow-x-auto">
            ...
            <h3 class="text-center font-bold text-slate-900 mb-4">CONTRIBUTIONS {{ $year }}</h3>

            @if($members->isEmpty())
            <div class="text-center py-12 text-slate-500">No contribution data for {{ $year }}.</div>
            @else
            <table class="w-full border-collapse border border-slate-300 text-sm min-w-[800px]">
                <thead>
                    <tr class="bg-slate-100">
                        ...
                        <th class="border border-slate-300 px-2 py-1.5 text-right font-semibold">JUN</th>
                        <th class="border border-slate-300 px-2 py-1.5 text-right font-semibold">JUL</th>
