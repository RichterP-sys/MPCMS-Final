...
<div class="space-y-6">
        ...
        <div class="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <h1 class="text-2xl font-bold text-white">Schedule of Loan Receivables</h1>
                </div>
                <p class="text-gray-100">For the Year Ended December 31, {{ $year }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('admin.reports.schedule.loans-receivable.export', ['year' => $year]) }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-white/15 hover:bg-white/25 border border-white/20 text-white text-sm font-medium rounded-xl">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden print:shadow-none">
        <div class="p-6 print:p-4">
            ...
            <h3 class="text-center font-bold text-slate-900 mb-2">SCHEDULE OF LOAN RECEIVABLES</h3>
            <p class="text-center text-sm text-slate-600 mb-4">For the Year Ended December 31, {{ $year }}</p>
            @if(!$blocks->isEmpty())
            ...
            @if($blocks->isEmpty())
            <div class="text-center py-12 text-slate-500">No outstanding loan receivables.</div>
            @else
